...
class GameApiViewTests( TestCase ):
    ...
    #tests the GET game_solution method in views.py by checking if it
    #returns the solution in json format that will match the self.mock_game.word
    def test_game_view_should_respond_with_solution( self ):
        with patch.object( Game.objects, 'get' ) as mock_get:
            mock_get.return_value = self.mock_game
            ...
            self.mock_game.word = {'solution' : 'TESTWORD'}
            self.assertEqual(response.data, self.mock_game.word)


    #tests the GET game_solution method to see if it will return with a 404 error
    #if the game id isn't found
    def test_game_view_should_respond_with_404_when_game_not_found( self ):
        with patch.object( Game.objects, 'get' ) as mock_get:
